import { findFarmerPDA, findRarityPDA } from "@gemworks/gem-farm-ts"
...
export interface FarmState {
  walletNFTs: NFT[]
  farmerAccount: any,
  farmerVaultAccount: any,
  farmerStatus: any,
  selectedWalletItems: NFT[],
  isLocked: boolean,
  availableA: any,
  feedbackStatus: string,
  handleStakeButtonClick: () => Promise<void>,
  handleUnstakeButtonClick: () => Promise<void>,
  handleClaimButtonClick: () => Promise<void>,
  handleWalletItemClick: (item: NFT) => void,
  handleMoveToVaultButtonClick: () => Promise<void>,
  handleInitStakingButtonClick: () => Promise<void>,
  farmerVaultNFTs: NFT[],
  selectedVaultItems: NFT[],
  handleMoveToWalletButtonClick: () => Promise<void>,
  handleVaultItemClick: (item: NFT) => void,
  handleRefreshRewardsButtonClick: () => Promise<boolean>,
}


const useGemFarmStaking = (farmId: string): FarmState => {
  const { connection } = useConnection()
  const wallet = useAnchorWallet() as SignerWalletAdapter
  const walletNFTs = useWalletNFTs().agentWalletNFTs
  ...
  const handleMoveToVaultButtonClick = async () => {
    ...
    for (const nft of selectedWalletItems) {
      ...
      // Set rarity points if needed
      const response = await fetch(`/api/updateStakingRarityPoints/${nft.onchainMetadata.mint}`)
